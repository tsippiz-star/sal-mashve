...
# בוחר את המסד הגדול ביותר: prices.db, "prices (2).db", prices_v2.db וכו'
_cands = sorted(ROOT.glob("prices*.db"), key=lambda x: x.stat().st_size, reverse=True)
_cands += [ROOT / "prices.db", ROOT.parent / "prices.db"]
DB_PATH = next((x for x in _cands if x.exists()), ROOT / "prices.db")
