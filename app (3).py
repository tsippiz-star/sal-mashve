...
# ─── טאבים ─────────────────────────────
# ─── בדיקה גלויה: כמה רשתות נטענו ומאיזה קובץ ───
try:
    _chk = sqlite3.connect(DB_PATH)
    _n_chains = _chk.execute("SELECT COUNT(DISTINCT chain) FROM prices").fetchone()[0]
    _chk.close()
except Exception:
    _n_chains = 0

st.caption(f"📁 קובץ נתונים שנטען: `{DB_PATH.name}` · רשתות בקובץ: **{_n_chains}**")
if _n_chains <= 1:
    st.error(
        "⚠️ הנתונים שנטענו מכילים רשת אחת בלבד — כנראה שהאפליקציה קוראת קובץ DB ישן. "
        "יש להעלות את הקובץ `prices.db` החדש (עם 8 הרשתות) ולחצות Reboot app."
    )
